...
getblock (int page)
{
  ...
  if (quick[page].s.ptr == quick + page)
	...
	return (NULL);		/* no space at all */
      ...
      int i;
      for (i = (getpagesize () / sizeof (Header)) / nunits; i > 0;
	   i--, p += nunits)
	{			/* slice it up into smaller blocks of fixed size */
	  p->s.size = nunits;
	  p->s.ptr = p + nunits;
	}
...
malloc (unsigned nbytes)
{
  ...
  for (p = prevp->s.ptr; p != freep; prevp = p, p = p->s.ptr)	/* iterate over all entries, find an apropriate entry, but remember its parent */
    if (CRITERIA)
      before = prevp;		/* best match so far? */
  if (CRITERIA)
    before = prevp;		/* previous loop misses last element */

  if (before->s.ptr->s.size < nunits)
    {				/* best match not a fit */
  ...
  for (p = prevp->s.ptr;; prevp = p, p = p->s.ptr)
    {				/* iterate over list items until match is found */
      if (p->s.size >= nunits)
	{			/* big enough */
	  if (p->s.size == nunits)	/* match is exact size */
	    prevp->s.ptr = p->s.ptr;
...
void *
realloc (void *old, unsigned nbytes)
{
  if (old == NULL)
  ...
  for (page = 0; (2 << page) < nunits; page++);	/* Check to see if there is a list for blocks of this size */
  if (page < NRQUICKLISTS)
    nunits = 2 << page;		/*New size is a fixed block size */
#endif
  diff = nunits - p->s.size;
  if (diff == 0)
    return old;			/*Same size, do nothing */
  if (diff < 0)
    {				/*Smaller size, resize the block and free the rest */
      prevp = p;
      prevp->s.size = nunits;
      p += nunits;
      p->s.size = -diff;
...
free (void *ap)
{
  ...
  if (page < NRQUICKLISTS && 2 << page == bp->s.size)
    {				/* free fixed size block */
      freeblock (bp, page);
      return;
    }
#endif
